...
public class cloudVillage
{
    private readonly DBRepository ___repo;

    public cloudVillage(DBRepository __repo)
    {
        ___repo = __repo;
    }
    ...
    {
    List<Product> product =___repo.GetAllCloudProducts();
    ...
    private void CartPrompt()
    {
        ...
        if(answer != null)
        {
            ...
            if(Char.ToUpper(answerC) == 'Y')
            {
                ...
                int number = ___repo.getCloudProduct(item);
                ...
                if (number>buy)
                {
                    ...
                    ___repo.UpdateCloudInventory(item, remaining);
